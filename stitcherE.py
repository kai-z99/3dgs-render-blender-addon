#!/usr/bin/env python3
"""
Stitch vertical equirectangular strips into a single equirect panorama.

Inputs: a folder containing N strips named like strip_000.png ... strip_{N-1}.png
(Any extension is fine: .png/.jpg/.tif/.tga/.hdr/.exr). The tool will also
accept arbitrary filenames and sort them by the first integer it finds.

Output: equirect.<ext> by default (same ext as input unless overridden).
"""
...
def _read(path: str):
    ...
    if ext in [".hdr", ".exr"]:
        ...
        if Image is None:
            raise RuntimeError("Reading non-HDR formats requires Pillow (pip install Pillow).")
...
def _write(path: str, arr):
    ...
    if a.dtype.kind == 'f':
        # Normalize floats into 0..1 -> 0..255
...
def _find_strips(folder: str, pattern_prefix="strip_"):
    # Prefer files named like strip_###.ext; else fall back to any supported ext
    entries = []
    for name in os.listdir(folder):
        low = name.lower()
        root, ext = os.path.splitext(low)
        if ext in SUPPORTED_EXTS:
            if root.startswith(pattern_prefix):
                entries.append(name)
    if not entries:
        # Fallback: any supported ext
...
def _unify_dtype_and_channels(imgs):
    # Make all the same channel count; prefer RGBA if any has alpha
...
def _edge_feat(im, side, k=8, down=256):
    a = im.astype(np.float32)
    if a.ndim == 2:
        a = a[..., None]
    k = min(k, a.shape[1])
    col = a[:, :k, :] if side == 'L' else a[:, -k:, :]
    feat = col.mean(axis=1)
    m, s = feat.mean(), feat.std() + 1e-6
    feat = (feat - m) / s
    if feat.shape[0] > down:
        idx = np.linspace(0, feat.shape[0]-1, down).astype(int)
        feat = feat[idx]
    return feat

def _auto_order(imgs):
    n = len(imgs)
    L = [_edge_feat(im, 'L') for im in imgs]
    R = [_edge_feat(im, 'R') for im in imgs]

    def seq_cost(order):
        return sum(((R[order[t]] - L[order[(t+1) % n]])**2).mean() for t in range(n))

    best = (float('inf'), None, False)  # (cost, order, is_reversed)

    base = list(range(n))
    for k in range(n):  # normal direction
        order = base[k:] + base[:k]
        c = seq_cost(order)
        if c < best[0]:
            best = (c, order, False)
    ...
    for k in range(n):  # reversed direction
        order = base_rev[k:] + base_rev[:k]
        c = seq_cost(order)
        if c < best[0]:
            best = (c, order, True)

    _, order, is_rev = best
    return [imgs[i] for i in order], order, is_rev


def main():
    ap = argparse.ArgumentParser(description="Stitch vertical equirectangular strips into a panorama.")
    ap.add_argument("--folder", "-f", default=".")
    ap.add_argument("--prefix", default="strip_", help="Filename prefix to match (default strip_)")
    ap.add_argument("--out", "-o", default="equirect")
    ...
    imgs, order, is_rev = _auto_order(imgs)
    print(f"[INFO] Auto-ordered strips -> {order}  (reversed={is_rev})")

    # Optionally crop small height/width mismatches to min size (avoids seam off-by-ones)
    min_h = min(im.shape[0] for im in imgs)
    # Widths may vary (last strip); we'll use each as-is and tile in order
    imgs = [im[:min_h, :im.shape[1], ...] for im in imgs]

    # Build output canvas (sum widths)
